...
void Shuffle_probs(double **array, size_t Nl) {
	...
	*/
    ...
    if (Nl > 1) {
        size_t i;
        for (i = 1; i < Nl - 1; i++) { /* from i=1 to Nl-1 because I'm reshuffling all but the rates in the borders */
          size_t j = i + rand() / (RAND_MAX / ((Nl) - i) + 1); /* check what Nl should be if we include borders! */
          left_prob = array[j][0];
...
int main(int argc, char *argv[]) {
	...
	/* We shuffle outside nrun loop, that's why it is QUENCHED */
	Shuffle_probs(transition_probs, Nl);

	fp = fopen(fout1, "w+");
	
	fprintf(fp, "pos rate-left rate-right\n");
	for (i = 0; i < Nl + 1; i++) {
		fprintf(fp, "%d %g %g\n", i, transition_probs[i][0], transition_probs[i][1]);
	}
	fprintf(fp, "\n\n");
