...
void TransProb_Voter(double left_boundary, double right_boundary, 
	...
	for (i = 0; i < Nl + 1; i++) {
		/* Not setting boundary conditions manually because they come out
		 * naturally from the functional form
		 * */
		pos_norm = left_boundary + 2 * i / (double)(Nl);
		/* left jump prob */
		transition_probs[i][0] = 0.25 * (1 - pos_norm * pos_norm); 
		/* right jump prob */
		transition_probs[i][1] = 0.25 * (1 - pos_norm * pos_norm);
	}
...
int main(int argc, char *argv[]) {
    ...
    int time_counter; /* counts num steps by the walker */
    int time_counter_max; /* max num steps allowed */
    int fptime; /* first passage time to target_boundary */
	...
	fp = fopen(finput, "r+");
	rewind(fp);
	fscanf(fp, "%d %lf %lf %lf %lf %d %s", &Nl, &initial_position, &target_position, 
											&left_boundary, &right_boundary, &nruns, fout1);
	fclose(fp);
	
	printf("%d %lf %lf %lf %lf %d %s\n", Nl, initial_position, target_position, 
											left_boundary, right_boundary, nruns, fout1);
	...
	TransProb_Voter(left_boundary, right_boundary, transition_probs, Nl);
	...
	for (run = 0; run < nruns; run++) {
		...
		//~ } while (walker_position == target_position);
		if (time_counter > time_counter_max) {
			time_counter = -1;
			break;
		}
		
		} while ((walker_position_index != 0)  &&
				 (walker_position_index != Nl) &&
